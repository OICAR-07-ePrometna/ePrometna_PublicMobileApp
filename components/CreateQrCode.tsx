...
import { View, Text, ScrollView, ActivityIndicator, SafeAreaView, TouchableOpacity, FlatList, TextInput } from 'react-native';
...
const CreateQrCode: React.FC = () => {
  ...
  const loadVehicles = async () => {
    try {
      ...
      setError(err.message || 'Neuspjelo učitavanje vozila');
      setScreenState('error');
    } finally {
      setLoading(false);
    }
  ...
  const generateQRCodeForVehicle = async (vehicle: VehicleDto) => {
    try {
      ...
      setError(err.message || 'Neuspjelo generiranje QR koda');
      setScreenState('error');
    } finally {
      setLoading(false);
    }
